...
AVL::RotateRight(NodePointerType<TData> root)
{
    ...
    root->Hight(std::max(root->Left()->Hight(), root->Right()->Hight()) + 1);
    root_left.Hight(std::max(root_left.Left()->Hight(), 
...
AVL::RotateLeft(NodePointerType<TData> root)
{
    ...
    root->Hight(std::max(root->Left()->Hight(), root->Right()->Hight()) + 1);
    root_right.Hight(std::max(root_right.Left()->Hight(), 
...
{
    if (!root || !*root) return nullptr;
    const auto res = compare_func(**root, data);
    if (res == 0)
        return root;
    else if (res == 1)
...
{
    if (swap->Left())
        EraseMaximum(root, swap->Left()->MakeCopy());
    else
    {
        root->Swap(*swap);
        Erase(swap);
